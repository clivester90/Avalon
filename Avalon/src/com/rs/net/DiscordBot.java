		...
		api.connect(new FutureCallback<>() {
			...
			public void onSuccess(DiscordAPI api) throws StringIndexOutOfBoundsException {
				...
				api.registerListener((MessageCreateListener) (api1, message) -> {
					Logger.log("DiscordBot", "Listener works");
					Logger.log("DiscordBot", "User: " + message.getAuthor().getName());
					Logger.log("DiscordBot", "Message: " + message.getContent());
					if (!message.isPrivateMessage()) {
						handleWarnings(message);
					}
					if (message.getContent().isEmpty()) {
						Logger.log("DiscordBot", "Message is empty");
						return;
					}
					if (message.isDeleted()) {
						Logger.log("DiscordBot", "Message is deleted");
						return;
					}
					if (message.getContent().charAt(0) != '!') {
						Logger.log("DiscordBot", "Message did not start with !");
						return;
					}
					Logger.log("DiscordBot", "Start doing command");
