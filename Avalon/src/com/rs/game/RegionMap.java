...
public class RegionMap {
	...
	public void setMask(int plane, int x, int y, int mask) {
		if (x >= 64 || y >= 64 || x < 0 || y < 0) {
			...
			if (clipedOnly)
				World.getRegion(tile.getRegionId()).forceGetRegionMapClipedOnly().setMask(plane,
						tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			else
				World.getRegion(tile.getRegionId()).forceGetRegionMap().setMask(plane, tile.getX() - newRegionX,
	...
	public void addMask(int plane, int x, int y, int mask) {
		if (x >= 64 || y >= 64 || x < 0 || y < 0) {
			...
			int newRegionY = (regionId & 0xff) * 64;
			if (clipedOnly)
				World.getRegion(tile.getRegionId()).forceGetRegionMapClipedOnly().addMask(plane,
						tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			else
				World.getRegion(tile.getRegionId()).forceGetRegionMap().addMask(plane, tile.getX() - newRegionX,
						tile.getY() - newRegionY, mask);
			return;
		}
	...
	public void removeMask(int plane, int x, int y, int mask) {
		if (x >= 64 || y >= 64 || x < 0 || y < 0) {
			...
			int newRegionY = (regionId & 0xff) * 64;
			if (clipedOnly)
				World.getRegion(tile.getRegionId()).forceGetRegionMapClipedOnly().removeMask(plane,
						tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			else
				World.getRegion(tile.getRegionId()).forceGetRegionMap().removeMask(plane, tile.getX() - newRegionX,
						tile.getY() - newRegionY, mask);
			return;
		}
		masks[plane][x][y] &= (~mask);
