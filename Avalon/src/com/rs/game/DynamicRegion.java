			...
			for (int dynX = 0; dynX < 8; dynX++) {
				for (int dynY = 0; dynY < 8; dynY++) {
					...
					int landArchiveId = Cache.STORE.getIndexes()[5]
							.getArchiveId("l" + (mapID >> 8) + "_" + (mapID & 0xFF));
					int mapArchiveId = Cache.STORE.getIndexes()[5]
							.getArchiveId("m" + (mapID >> 8) + "_" + (mapID & 0xFF));
					byte[] mapContainerData = mapArchiveId == -1 ? null
							: Cache.STORE.getIndexes()[5].getFile(mapArchiveId, 0);
					byte[] landContainerData = landArchiveId == -1 ? null
							: Cache.STORE.getIndexes()[5].getFile(landArchiveId, 0, MapArchiveKeys.getMapKeys(mapID));
								...
								for (int y = 0; y < 64; y++) {
									if ((mapSettings[z][x][y] & 0x1) == 1) {
										...
										if (realZ == renderChunkZ && (x >> 3) == renderLocalChunkX
											...
											forceGetRegionMap().addUnwalkable(dynZ, (dynX << 3) | coords[0],
													(dynY << 3) | coords[1]);
										}
									}
								}
