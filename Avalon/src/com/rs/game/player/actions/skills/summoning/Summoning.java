package com.rs.game.player.actions.skills.summoning;
...
public class Summoning {
	...
	public static enum Pouch {
		...
		IRON_MINOTAUR(149, 12075, 4.6, 104.8, 2220000, 9),
	...
	public static void handlePouchInfusion(Player player, int slotId, int creationCount) {
		...
		if (pouch == null)
			return;
		boolean infusingScroll = (boolean) player.temporaryAttribute().remove("infusing_scroll"),
		...
		if (itemReq != null) {
			...
			double totalXp = 0;
			totalXp = (infusingScroll ? pouch.getMinorExperience() : pouch.getExperience()) * amount;
			hasRequirements = true;
			player.getInventory()
					.addItem(new Item(infusingScroll ? getScrollId(pouch.getRealPouchId()) : pouch.getRealPouchId(),
							(infusingScroll ? 12 : 1) * amount));
			player.getSkills().addXp(Skills.SUMMONING, totalXp);
		}
		if (!hasRequirements) {
			player.temporaryAttribute().put("infusing_scroll", infusingScroll);
			return;
		}
		player.closeInterfaces();
		player.animate(POUCH_INFUSION_ANIMATION);
		player.gfx(POUCH_INFUSION_GRAPHICS);
	}

	public static void switchInfusionOption(Player player) {
		boolean infusingScroll = (boolean) player.temporaryAttribute().get("infusing_scroll");
		if (infusingScroll)
