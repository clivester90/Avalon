...
public class Nomad extends NPC {
	...
	public void sendTeleport(final WorldTile tile) {
		animate(new Animation(12729));
		gfx(new Graphics(1576));
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				setNextWorldTile(tile);
				animate(new Animation(12730));
	...
	public WorldTile getThroneTile() {
		 ...
		 */
		return throneTile == null ? new WorldTile((getRegionX() << 6) + 32, (getRegionY() << 6) + 32, getPlane())
				: throneTile;
	}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (int i = 0; i < 4; i++) {
					...
					n.animate(new Animation(12730));
					n.gfx(new Graphics(1577));
					n.setTarget(target);
				}
