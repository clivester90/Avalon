...
public class Class481_Sub1 extends Class481 {
	...
	try {
	    ...
	    if (null == string_0_)
		outputstream.write(new StringBuilder().append("CONNECT ").append(this.aString6034).append(":").append(98195689 * this.anInt6033).append(" HTTP/1.0\n\n").toString().getBytes(Charset.forName("ISO-8859-1")));
	    else
		outputstream.write(new StringBuilder().append("CONNECT ").append(this.aString6034).append(":").append(this.anInt6033 * 98195689).append(" HTTP/1.0\n").append(string_0_).append("\n\n").toString().getBytes(Charset.forName("ISO-8859-1")));
	    outputstream.flush();
	...
	try {
		...
		try {
			...
			if (((Boolean) method_6_.invoke(object, new Object[0])).booleanValue()) {
			    ...
			    String string_10_ = ((String) (method_8_.invoke(object, (new Object[] { new URL(new StringBuilder().append("https://").append(this.aString6034).append("/").toString()), "https" }))));
		...
		socket.connect(new InetSocketAddress((this.aString6034), (98195689 * (this.anInt6033))));
		return socket;
	    }
	    return null;
	}
	...
	boolean bool_11_ = this.anInt6033 * 98195689 == 443;
	List list;
	List list_12_;
	try {
	    list = (this.aProxySelector8445.select(new URI(new StringBuilder().append(bool_11_ ? "https" : "http").append("://").append(this.aString6034).toString())));
	    list_12_ = (this.aProxySelector8445.select(new URI(new StringBuilder().append(bool_11_ ? "http" : "https").append("://").append(this.aString6034).toString())));
	}
	...
	try {
	    ...
	    if (!bool)
		System.setProperty("java.net.useSystemProxies", "true");
	    boolean bool_16_ = this.anInt6033 * 98195689 == 443;
	    List list;
	    List list_17_;
	    try {
		list = (this.aProxySelector8445.select(new URI(new StringBuilder().append(bool_16_ ? "https" : "http").append("://").append(this.aString6034).toString())));
		list_17_ = (this.aProxySelector8445.select(new URI(new StringBuilder().append(bool_16_ ? "http" : "https").append("://").append(this.aString6034).toString())));
	    }
	    catch (URISyntaxException urisyntaxexception) {
