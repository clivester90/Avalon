...
public class Class298_Sub44 extends Class298 {
	...
	try {
	    ...
	    buffer.writeByte((this.anInt7471 * -1570985485)); // OS
							      // type.
	    buffer.writeByte((this.aBoolean7509 ? 1 : 0)); // 64-bit
	    buffer.writeByte((-1414570243 * this.anInt7486)); // OS
							      // version
	    buffer.writeByte((this.anInt7491 * 43286197)); // Java
							   // vendor
	    buffer.writeByte(anInt7492 * 154600941); // Java version
	    buffer.writeByte((-1529246497 * this.anInt7493)); // Java
							      // sub
							      // version
	    buffer.writeByte(anInt7494 * 1324779323); // Java sub sub version.
	    buffer.writeByte((this.aBoolean7495 ? 1 : 0)); // Always
							   // false
	    buffer.writeShort((this.anInt7469 * -2088235843), 16711935); // Maximum
									 // memory
	    buffer.writeByte((-1837764033 * this.anInt7498)); // Available
							      // processors
	    buffer.putTriByte(399637415 * cpuClockSpeed, (byte) 115); // Clock
								      // speed
								      // (MHz)
	    buffer.writeShort((-1360789919 * this.cpuClockSpeed1), 16711935); // Clock
									      // speed
									      // (MHz)
	    ...
	    buffer.writeByte((this.directXDriverDate * 1183412533)); // DirectX
								     // driver
								     // date
								     // (month).
	    buffer.writeShort((-669549295 * this.directXDriverData2), 16711935); // DirectX
										 // driver
										 // date
										 // (year).
	    buffer.putJagString(this.aString7496, (short) 3988); // CPU
								 // type.
	    buffer.putJagString(this.aString7511, (short) -4559); // CPU
								  // data
	    buffer.writeByte((78038867 * this.cpuInfo)); // CPU
							 // cores
							 // amount.
	    buffer.writeByte((this.anInt7489 * 664924581)); // Raw
							    // cpu
							    // info0
	...
	try {
	    int i_1_ = 38;
	    i_1_ += Class120.method1310(this.aString7502, (byte) -1);
	    i_1_ += Class120.method1310(this.aString7503, (byte) -1);
	    i_1_ += Class120.method1310(this.aString7482, (byte) -1);
	    i_1_ += Class120.method1310(this.aString7505, (byte) -1);
	    i_1_ += Class120.method1310(this.aString7496, (byte) -1);
	    i_1_ += Class120.method1310(this.aString7511, (byte) -1);
	    return i_1_;
	}
	catch (RuntimeException runtimeexception) {
	...
	if (bool) {
		...
		this.aBoolean7509 = true;
	    else
		this.aBoolean7509 = false;
	    ...
	    this.anInt7493 = i_2_ * 1167404831;
		...
		if (is_6_ != null && is_6_.length % 5 == 0) {
			...
			class298_sub53.writeLEInt((272393713 * (class472.anInt5964)), -487042280);
			class298_sub53.writeLEInt((634425393 * (class472.anInt5963)), 164405285);
			...
			this.anInt7489 = -1403810259 * (i_14_ >> 16 & 0xff);
			...
			if (null != class472_17_) {
			    ...
			    class298_sub53.writeLEInt((class472_17_.anInt5963 * 634425393), 293591119);
			    class298_sub53.writeLEInt((272393713 * (class472_17_.anInt5964)), -1803597625);
			}
		    }
		    class298_sub53.index = 0;
		    this.aString7511 = class298_sub53.readString(-1784035682);
		}
	...
	if (null == this.aString7505)
	    this.aString7505 = "";
	if (this.aString7496 == null)
	    this.aString7496 = "";
	if (this.aString7511 == null)
	    this.aString7511 = "";
